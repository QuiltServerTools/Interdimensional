...
class SingleBiomeResult(private val element: BiomeSourceElement, private val biome: Biome) : BiomeSourceResult(element) {
    override fun getItemStack(): ItemStack {
        return ItemStack(getItem(biome.category)).setCustomName(element.handler.player.server.registryManager.get(Registry.BIOME_KEY).getId(biome)?.path?.text())
    }
