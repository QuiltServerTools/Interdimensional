...
public class CustomAreaHelper extends PortalFrameTester {
    ...
    private void countExistingPortalBlocks() {
        for (int i = 0; i < width; i++)
            for (int j = 0; j < height; j++)
                if (InterdimensionalPortals.isInstanceOfCustomPortal(world.getBlockState(this.lowerCorner.offset(negativeDir, i).up(j))))
                    foundPortalBlocks++;
    }
    ...
    public static boolean validStateInsidePortal(BlockState blockState, HashSet<Block> foundations) {
        ...
        if (blockState.isAir() || InterdimensionalPortals.isInstanceOfCustomPortal(blockState))
            return true;
    ...
    public void createPortal(Block frameBlock) {
        ...
        BlockState blockState = InterdimensionalPortals.blockWithAxis(link != null ? link.getPortalBlock().getDefaultState() : InterdimensionalPortals.getDefaultPortalBlock().getDefaultState(), axis);
